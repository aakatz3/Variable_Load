...
void DoPid()
{
	...
	// setPoint is a voltage. We need to convert that to counts.
    gateCounts = setPoint/4;
    if (gateCounts > GateVoltage_DVDAC_MAX_VALUE)
        gateCounts = GateVoltage_DVDAC_MAX_VALUE;
    ...
    GateVoltage_SetValue(gateCounts);
}
